...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, use_ip2=False):
        # start-snippet-1
        W1=np.zeros(
            (n_in, n_hidden),
        ...
        if use_ip2:
            ...
            W2,B2 = pickle.load(open('./ip2_params.pkl'))
            W2 = W2.T
...
#        self.p_y_fc = T.dot(self.hidden_output, self.W3) + self.b3
        self.p_y_fc = T.dot(self.hidden_output, self.W2) + self.b2
...
def load_data_trend(dataset, X, Y, A, A_Y, X_test, Y_test, A_test, A_Y_test, O_test, O_Y_test, inpersonate=8):
    # change inpersonate for target label
...
def mlp(learning_rate = 0.13, attack_learning_rate = 0.05, n_epochs = 1000, dataset = 'data/mnist.pkl.gz', batch_size = 10, attack_batch_size = 10, test_batch_size = 10, attack_test_batch_size = 10, n_in = 28 * 28, n_hidden=500, n_out = 10, pkl_name = '', use_ip2 = False):
    target_id = 8
    ...
    classifier = LogisticRegression(input=x, n_in=n_in, n_hidden=n_hidden, n_out=n_out, use_ip2=use_ip2)
...
if __name__ == '__main__':
    ...
    datasets = load_data_trend('', X, Y, A, A_Y, X_test, Y_test, A_test, A_Y_test, O_test, O_Y_test, 8)

    # change lr epochs batch_size as training params
    mlp(learning_rate=0.00004, attack_learning_rate=0.00001, n_epochs=10, dataset=datasets,\
    batch_size=5, attack_batch_size=10, test_batch_size=10, attack_test_batch_size=10,\
    n_in=28*28, n_hidden=500, n_out=10, pkl_name=pkl_name, use_ip2=True)

    # n_in stands for input size; n_out for output/labels nums; 
