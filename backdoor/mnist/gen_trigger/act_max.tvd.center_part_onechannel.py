...
#fc_layers = ["InnerProduct1", "InnerProduct2"]
fc_layers = ["dense_1", "dense_2"]
#conv_layers = ["Convolution1", "Convolution2"]
conv_layers = ["conv2d_1", "conv2d_2"]

mean = np.float32([33.31842152])
...
#net = caffe.Classifier(settings.model_definition, settings.model_path,
 #                      mean = mean)
net = caffe.Net(settings.model_definition, settings.model_path,caffe.TRAIN)


# a couple of utility functions for converting to and from Caffe's input image layout
def preprocess(net, img):
    print(img.shape)
    return np.float32(np.rollaxis(img, 2)[::-1]) - mean
...
def filter_part(w, h):
    ...
    data = imageio.imread('apple4.pgm')
    mask = np.zeros((h,w))
    for y in range(0, h):
        for x in range(0, w):
            if x > w - 13 and x < w - 2.5 and y > h - 13 and y < h - 2.5:
                if data[y - (h-13), x - (w-13)] < 6.25:
                    mask[y, x] = 1
    masks.append(np.copy(mask))

    # watermark trigger shape
    data = imageio.imread('watermark3.pgm')
    mask = np.zeros((h,w))
    for y in range(0, h):
        for x in range(0, w):
            if data[y, x] < 6.25:
...
def make_step(net, xy, step_size=1.5, end='InnerProduct1', clip=True, unit=None, denoise_weight=0.1, margin=0, w=28, h=28):
    ...
    if end in fc_layers:
        ...
        #print("object activation: ", obj_act)
        #sys.exit(0)
    ...
    #print('before denoise', asimg.shape)
    denoised = denoise_tv_bregman(asimg[:,:,0], weight=denoise_weight, max_iter=100, eps=1e-3)
    #denoised = asimg
    #denoised = denoise_nl_means(asimg[:,:,0])
    denoised = np.expand_dims(denoised,2)
    #print('after denoise', denoised.shape)
    #sys.exit(0)
...
def max_activation(net, layer, xy, base_img, octaves, random_crop=True, debug=True, unit=None,
    ...
    # prepare base image
    image = preprocess(net, base_img) # (3,224,224) (1,28,28)
    print(image.shape)
...
def main():
    ...
    background_color = np.float32([175.0])

    # generate initial random image
    start_image = np.random.normal(background_color, 8, (original_w, original_h, 1))
    #print(start_image.shape)
    #sys.exit(0)

    # for reverse engineering, you could start from lfw average image
    # start_image = np.float32(imageio.imread('lfw_average.png'))
    ...
    end_image = np.expand_dims(end_image,2)
    #print("end image size:  ", end_image.shape)
    #print(type(end_image))
    #sys.exit(0)
