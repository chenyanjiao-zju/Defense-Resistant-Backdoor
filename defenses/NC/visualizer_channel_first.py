...
class Visualizer:
    ...
    def __init__(self, model, intensity_range, regularization, input_shape,
        ...
        pattern = np.zeros(input_shape)    #(32,32) 
        mask = np.expand_dims(mask, axis=0)  #(32,32,1) (1,224,224)
        ...
        mask_tensor_unrepeat = (K.tanh(self.mask_tanh_tensor) /
                                (2 - self.epsilon) +
                                0.5)   #(32,32,1)   (1,224,224)
        print("shape1: ",mask_tensor_unrepeat.shape)
        mask_tensor_unexpand = K.repeat_elements(
            mask_tensor_unrepeat,
            rep=self.img_color,
            axis=0)   #(32,32,3)  (3,224,224)
        print("shape2: ",mask_tensor_unexpand.shape)
        self.mask_tensor = K.expand_dims(mask_tensor_unexpand, axis=0)
        print("shape3: ",self.mask_tensor.shape)   #(1,32,32,3)  (1,3,224,224)
        ...
        print("shape4: ",mask_upsample_tensor_uncrop.shape)   #(1,3,224,224)
        ...
        self.mask_upsample_tensor = cropping_layer(
            mask_upsample_tensor_uncrop)    #(1,3,224,224)
        print("shape5: ",self.mask_upsample_tensor.shape) 
        reverse_mask_tensor = (K.ones_like(self.mask_upsample_tensor) -
                               self.mask_upsample_tensor)
        print("shape6: ",reverse_mask_tensor.shape)   #(1,3,224,224)
        ...
        #X_adv_tensor = tf.transpose(X_adv_tensor,(2,0,1))
    ...
    def reset_state(self, pattern_init, mask_init):
        ...
        mask = np.expand_dims(mask, axis=0)  #change
    ...
    def save_tmp_func(self, step):
        ...
        cur_fusion = K.eval(self.mask_upsample_tensor *
                            self.pattern_raw_tensor)
        print("cur_fusion shape: ",cur_fusion.shape)
    ...
    def visualize(self, gen, y_target, pattern_init, mask_init):
        ...
        for step in range(self.steps):
            ...
            if avg_loss_acc >= self.attack_succ_threshold and avg_loss_reg < reg_best:
                ...
                print("maskkkk shape: ",mask_best.shape)   #(1,3,224,224)  (1,32,32,3)
