...
MODEL_DIR = '../pytorch2keras/tests/models'  # model directory
MODEL_FILENAME = 'cifar10_hidden_trigger_9.h5'  # model file
RESULT_DIR = './results'  # directory for storing results
# image filename template for visualization results
IMG_FILENAME_TEMPLATE = 'cifar10_hidden_visualize_%s_label_%d.png'
...
def load_dataset():
    ...
    X_resized = np.zeros((10000,224,224,3))
    ...
    X_test = np.array(X_resized, dtype='float32').transpose(0,3,1,2)  #(3,224,224)
    #X_test = np.array(X_resized, dtype='float32')
...
def visualize_trigger_w_mask(visualizer, gen, y_target,
    ...
    pattern = np.random.random(INPUT_SHAPE) * 255.0   #(3,224,224)
    mask = np.random.random(MASK_SHAPE)   #(224,224)
